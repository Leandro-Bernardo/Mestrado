...
class IronOxidNetwork(ContinuousNetwork):
...
class IronOxidEstimationFunction(EstimationFunction):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(IronOxidNetwork, **kwargs)
    ...
    def _check_and_reshape_calibrated_pmf(self, **kwargs: CalibratedDistributions) -> Tuple[torch.Tensor, ...]:
        ...
        for name, calibrated_pmf in kwargs.items():
            if calibrated_pmf is None or not isinstance(calibrated_pmf, torch.Tensor) or calibrated_pmf.dtype != torch.float32 or calibrated_pmf.dim() != 2 or calibrated_pmf.shape[-1:] != (511,):
                raise ValueError(f"The {name} argument is not a valid batch of calibrated distribution with shape (batch_size, 511)")
    ...
    def _check_and_reshape_pmfs(self, **kwargs: Distribution) -> Tuple[torch.Tensor, ...]:
        ...
        for name, pmf in kwargs.items():
            if pmf is None or not isinstance(pmf, torch.Tensor) or pmf.dtype != torch.float32 or pmf.dim() != 1 or pmf.shape[-1:] != (511,):
                raise ValueError(f"The {name} argument is not a valid distribution with shape (511,)")
            size, = pmf.shape
