from .._model import *
from ..typing import CalibratedDistributions, Distribution, Prediction, ExtractedFeatures
...
class SulfateNetwork(ContinuousNetwork):
...
class SulfateUpNetwork(ContinuousUpNetwork):
    def __init__(self, backbone: Module, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.model = torch.nn.Sequential(OrderedDict([
            ("backbone", backbone),
...
class SulfateNetworkSqueezeNetStyle(SulfateNetwork):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(backbone=SqueezeNet1_1_1d(in_channels=1, num_classes=1), **kwargs)


class SulfateNetworkVgg11Style(SulfateNetwork):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(backbone=Vgg11_1d(in_channels=1, num_classes=1), **kwargs)

class SulfateNetworkUpVgg11Style(SulfateUpNetwork):
    def __init__(self, input_roi: Tuple[Tuple,...], in_channels: int, **kwargs: Any) -> None:
        super().__init__(backbone=UpVgg11_1d(in_roi=input_roi, in_channels=in_channels, num_classes=1), **kwargs)

class SulfateNetworkVgg19Style(SulfateNetwork):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(backbone=Vgg19_1d(in_channels=1, num_classes=1), **kwargs)
...
class PreProcessing(Module):
    def __init__(self, *, input_range: Tuple[float, float], input_roi: Tuple[Tuple[int, int]], **_) -> None:
        super().__init__()
    ...
    def __repr__(self) -> str:
        return f"{self.__class__.__name__}(input_range={tuple(map(float, self.input_range))}, input_roi={tuple(map(int, self.input_roi))})"
    ...
    def forward(self, calibrated_pmf: CalibratedDistributions) -> torch.Tensor:
        ...
        #roi = normalized[..., self.input_roi[0] : self.input_roi[1] + 1]
        roi = normalized[..., self.input_roi[0, 0]:self.input_roi[0, 1] + 1]
        return roi.unsqueeze(1)


class SulfateEstimationFunction(EstimationFunction):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(SulfateNetwork, **kwargs)
    ...
    def _check_and_reshape_calibrated_pmf(self, **kwargs: CalibratedDistributions) -> Tuple[torch.Tensor, ...]:
        ...
        for name, calibrated_pmf in kwargs.items():
            if calibrated_pmf is None or not isinstance(calibrated_pmf, torch.Tensor) or calibrated_pmf.dtype != torch.float32 or calibrated_pmf.dim() != 2:
                raise ValueError(f"The {name} argument is not a valid batch of calibrated distribution with shape (batch_size, size)")
    ...
    def _check_and_reshape_pmfs(self, **kwargs: Distribution) -> Tuple[torch.Tensor, ...]:
        ...
        for name, pmf in kwargs.items():
            if pmf is None or not isinstance(pmf, torch.Tensor) or pmf.dtype != torch.float32 or pmf.dim() != 1:
                raise ValueError(f"The {name} argument is not a valid distribution with shape (size,)")
            size, = pmf.shape
            result.append(pmf.view(1, size))
        return tuple(result)
