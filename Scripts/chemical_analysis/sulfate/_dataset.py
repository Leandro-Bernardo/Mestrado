...
class SulfateSampleDataset(SampleDataset):
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        super().__init__(*args, **kwargs)

    def _parse_auxiliary_solutions(self, raw_sample: Dict[str, Any]) -> List[AuxiliarySolution]:
        return [
            self._parse_auxiliary_solution("Reagente", raw_sample["sulfateReagent"]),
        ]
    
    def _parse_values(self, raw_sample: Dict[str, Any]) -> Tuple[Optional[float], Optional[float], str]:
        return (
            raw_sample["sourceStock"].get("sulfateConcentration", None),
            raw_sample.get("estimatedConcentration", None),
            raw_sample["sourceStock"].get("sulfateConcentrationUnit", "MILLIGRAM_PER_LITER_OF_SULFATE"),
        )


class ProcessedSulfateSampleDataset(ProcessedSampleDataset):
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        super().__init__(*args, **kwargs)
