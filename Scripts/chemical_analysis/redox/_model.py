...
class RedoxNetwork(ContinuousNetwork):
...
class PostProcessing(Module):
    ...
    def forward(self, normalized_redox: torch.Tensor) -> Prediction:
        normalized_redox = normalized_redox.squeeze(-1)
        redox = (2.0 * normalized_redox - 1.0) * (2.5758 * 1.4826 * self.training_mad) + self.training_median
        return redox, normalized_redox
...
class RedoxEstimationFunction(EstimationFunction):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(RedoxNetwork, **kwargs)
