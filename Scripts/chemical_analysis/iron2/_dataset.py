...
class Iron2SampleDataset(SampleDataset):
    ...
    def _parse_auxiliary_solutions(self, raw_sample: Dict[str, Any]) -> List[AuxiliarySolution]:
        return [
            ...
            self._parse_auxiliary_solution("Buffer", raw_sample["buffer"]),
        ]
    
    def _parse_values(self, raw_sample: Dict[str, Any]) -> Tuple[Optional[float], Optional[float], str]:
        return (
            raw_sample["sourceStock"].get("iron2Concentration", None),
            raw_sample.get("estimatedConcentration", None),
            raw_sample["sourceStock"].get("iron2ConcentrationUnit", "MILLIGRAM_PER_LITER_OF_IRON2"),
        )


class ProcessedIron2SampleDataset(ProcessedSampleDataset):
