...
class Bisulfite2DSampleDataset(SampleDataset):
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        super().__init__(*args, **kwargs)

    def _parse_auxiliary_solutions(self, raw_sample: Dict[str, Any]) -> List[AuxiliarySolution]:
         return [ 
            self._parse_auxiliary_solution("bisulfiteReagent", raw_sample["bisulfiteReagent"]),
            self._parse_auxiliary_solution("formaldehyde", raw_sample["formaldehyde"]),
        ]
    
    def _parse_values(self, raw_sample: Dict[str, Any]) -> Tuple[Optional[float], Optional[float], str]:
        return (
            raw_sample["sourceStock"].get("bisulfiteConcentration", None),
            raw_sample.get("estimatedConcentration", None),
            raw_sample["sourceStock"].get("bisulfiteConcentrationUnit", "MILLIGRAM_PER_LITER_OF_BISULFITE"),
        )


class ProcessedBisulfite2DSampleDataset(ProcessedSampleDataset):
