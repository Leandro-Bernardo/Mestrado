...
class PhNetwork(ContinuousNetwork):
...
class PhUpNetwork(ContinuousUpNetwork):
    def __init__(self, backbone: Module, **kwargs: Any) -> None:
        super().__init__(**kwargs)
        self.model = torch.nn.Sequential(OrderedDict([
            ("backbone", backbone),
...
class PhNetworkMobileNetV3Style(PhNetwork):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(backbone=MobileNetV3Small(in_channels=1, num_classes=1), **kwargs)


class PhNetworkSqueezeNetStyle(PhNetwork):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(backbone=SqueezeNet1_1(in_channels=1, num_classes=1), **kwargs)


class PhNetworkShuffleNetV2X10Style(PhNetwork):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(backbone=ShuffleNetV2X10(in_channels=1, num_classes=1), **kwargs)


class PhNetworkShuffleNetV2X15Style(PhNetwork):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(backbone=ShuffleNetV2X15(in_channels=1, num_classes=1), **kwargs)


class PhNetworkShuffleNetV2X20Style(PhNetwork):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(backbone=ShuffleNetV2X20(in_channels=1, num_classes=1), **kwargs)


class PhNetworkVgg11Style(PhNetwork):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(backbone=Vgg11(in_channels=1, num_classes=1), **kwargs)

class PhNetworkUpVgg11Style(PhUpNetwork):
...
class PostProcessing(Module):
    ...
    def forward(self, normalized_ph: torch.Tensor) -> Prediction:
        normalized_ph = normalized_ph.squeeze(-1)
        ph = (2.0 * normalized_ph - 1.0) * (2.5758 * 1.4826 * self.training_mad) + self.training_median
        return ph, normalized_ph
...
class PhEstimationFunction(EstimationFunction):
    def __init__(self, **kwargs: Any) -> None:
        super().__init__(PhNetwork, **kwargs)
