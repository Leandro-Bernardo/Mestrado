...
for i in range(int(len(eval_loader)/(112*112))):
    ...
    plt.ylabel('Count')
    plt.savefig(os.path.join(EVALUATION_ROOT, "histogram", MODEL_VERSION, f"sample_{i}.png"))
    plt.close('all')
...
for i, image in enumerate(partial_loss_from_cnn1_output):
    plt.imsave(os.path.join(EVALUATION_ROOT, "error_from_image","from_cnn1_output", MODEL_VERSION, f"sample_{i}.png"), image)
...
for i, error_map in enumerate(partial_loss_from_original_image):
    ...
    plt.savefig(os.path.join(EVALUATION_ROOT, "error_from_image", "from_original_image", MODEL_VERSION, f"sample_{i}.png"))
    plt.close('all')
