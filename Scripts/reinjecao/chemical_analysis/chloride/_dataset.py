...
class ChlorideSampleDataset(SampleDataset):
    def __init__(self, *args: Any, **kwargs: Any) -> None:
        super().__init__(*args, **kwargs)

    def _parse_auxiliary_solutions(self, raw_sample: Dict[str, Any]) -> List[AuxiliarySolution]:
        return [
            self._parse_auxiliary_solution("Reagente", raw_sample["chlorideReagent"]),
            self._parse_auxiliary_solution("Liquidante", raw_sample["liquidator"]),
        ]

    def _parse_values(self, raw_sample: Dict[str, Any]) -> Tuple[Optional[float], Optional[float], str]:
        return (
            raw_sample["sourceStock"].get("chlorideConcentration", None),
            raw_sample.get("estimatedConcentration", None),
            raw_sample["sourceStock"].get("chlorideConcentrationUnit", "MILLIGRAM_PER_LITER_OF_SODIUM_CHLORIDE"),
        )


class ProcessedChlorideSampleDataset(ProcessedSampleDataset):
    def __init__(self, *args: Any, lab_mean: np.ndarray, lab_sorted_eigenvectors: np.ndarray, **kwargs: Any) -> None:
        ...
        super().__init__(*args, **kwargs)
        self._chloride_values = self.get_samples()

    @property
    def analyte_values(self):
        return self._chloride_values
